...
<div class="gestok-form-card">
    ...
    <div class="gestok-form-body">
        <form action="{{ route('stores.debitnotes.store', $store->id) }}" method="POST" id="debitNoteForm">
            @csrf
            ...
            <div id="saleDetailsSection" class="sale-details-section" style="display: none;">
                <h4>Seleccionar productos a adebitar</h4>
                <p class="text-muted">Ingresa las cantidades que deseas adebitar de cada producto:</p>
                ...
                <div id="totalsSummary" style="margin-top: 1rem; padding: 1rem; background: #f0f0f0; border-radius: 5px; display: none;">
                    ...
                    <strong>Total a adebitar: $<span id="summaryTotal">0.00</span></strong>
                </div>
            </div>

            <!-- Fecha de emisión -->
            <label for="debit_note_date">Fecha de emisión *</label>
            <input type="date" id="debit_note_date" name="debit_note_date" value="{{ old('debit_note_date', now()->format('Y-m-d')) }}" required>
            @error('debit_note_date')
            <div class="text-danger">{{ $message }}</div>
            @enderror

            <!-- Motivo -->
            <label for="reason">Motivo de la nota de Debito *</label>
            ...
            <div class="gestok-form-actions">
                <button type="submit" class="btn" id="submitBtn" disabled>Crear Nota de Debito</button>
                <a href="{{ route('stores.debitnotes.index', $store->id) }}" class="btn btn-secondary">Cancelar</a>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    document.getElementById('debitNoteForm').addEventListener('submit', function(e) {
        if (!Array.from(document.querySelectorAll('.quantity-field')).some(f => parseFloat(f.value) > 0)) {
            e.preventDefault();
            alert('Debe seleccionar al menos un producto y cantidad para adebitar.');
        }
    });
</script>
