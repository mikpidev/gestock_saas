...
<div class="card-table">
    <div class="card-header-custom">
        ...
        <a href="{{ route('stores.debitnotes.create', $store->id) }}" class="btn-new">
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $debitNote->sale->codigo_generacion ?? 'N/A' }}</td>
                ...
                <td>
                    ...
                    if ($debitNote->dte_status === 'PROCESADO') $statusClass = 'badge-procesado';
                    if ($debitNote->dte_status === 'RECHAZADO') $statusClass = 'badge-rechazado';
                    @endphp

                    @if($debitNote->dte_status === null || $debitNote->dte_status === 'PENDIENTE')
                    <form action="{{ route('stores.debitnotes.refreshDTE', [$store->id, $debitNote->id]) }}"
                        ...
                        <button class="badge {{ $statusClass }}" title="Actualizar DTE">
                            {{ $debitNote->dte_status ?? 'PENDIENTE' }}
                        </button>
                    </form>
                    @else
                    <span class="badge {{ $statusClass }}">
                        {{ $debitNote->dte_status }}
                    </span>
                    @endif
                </td>
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li>
                                <button class="dropdown-item"
                                    onclick="mostrarDTE('{{ route('dte.public', $debitNote->codigo_generacion) }}')">
                            ...
                            <li>
                                <button class="dropdown-item"
                                    onclick="mostrarModalImpresion('{{ route('ticket.print', [$store->id, $debitNote->id]) }}')">
                            ...
                            <li>
                                <button class="dropdown-item"
                                    onclick="enviarDTEPorCorreo('{{ route('stores.email.send', [$store->id, $debitNote->id]) }}')">
                            ...
                            <li>
                                <form action="{{ route('stores.debitnotes.destroy', [$store->id, $debitNote->id]) }}"
                                    method="POST"
                                    onsubmit="return confirm('¿Eliminar Nota de Debito?');">
                                    @csrf
                                    @method('DELETE')
...
<div class="modal fade" id="filtros" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-rm gradient-text" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btn-sm btn-edit" onclick="document.querySelector('.filters').submit()">Filtrar</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="reportes" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="GET" class="filter-bar" action="{{ route('reportes.notasdebito') }}">
                    <label>Desde:</label>
                ...
                <div class="modal-footer">
                    <a href="{{ route('reportes.notasdebito', ['from' => $dateFrom, 'to' => $dateTo]) }}">
                        <button type="button" class="btn btn-sm btn-edit" onclick="document.querySelector('.filter-bar').submit()">Generar Reporte</button>
                    </a>
                </div>
            </div>
        </div>
